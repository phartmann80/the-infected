...
import { withTempImageGenerationDir } from './fixtures';

describe('provenance record', () => {
  it('produces provenance JSON with all required fields on dry-run', async () => {
    await withTempImageGenerationDir('test-provenance-fields', async outDir => {
      const res = await run({
        provider: 'mock',
      ...
      const required = [
        ...
        'aspect_ratio', 'input_file_hashes', 'output_directory', 'status', 'canonical', 'dry_run',
      ];
      for (const k of required) {
        expect(content[k]).toBeDefined();
      }
      expect(content.output_directory).toBe(outDir);
    });
  }, 20000);

  it('records UTC timestamp', async () => {
    await withTempImageGenerationDir('test-provenance-date', async outDir => {
  ...
  it('records input file hash when reference is provided', async () => {
    await withTempImageGenerationDir('test-provenance-input-hash', async outDir => {
      const res = await run({
        provider: 'mock',
        brief: 'hero-key-art-001',
        reference: 'assets/visual_benchmarks/Visual_Benchmark_Candidate_001.png',
  ...
  it('defaults to internal-review status', async () => {
    await withTempImageGenerationDir('test-provenance-status', async outDir => {
      const res = await run({ provider: 'mock', brief: 'hero-key-art-001', output: outDir, dryRun: true });
      const content = JSON.parse(fs.readFileSync(res.provenance, 'utf8'));
  ...
  it('defaults canonical to false', async () => {
    await withTempImageGenerationDir('test-provenance-canonical', async outDir => {
      const res = await run({ provider: 'mock', brief: 'hero-key-art-001', output: outDir, dryRun: true });
      const content = JSON.parse(fs.readFileSync(res.provenance, 'utf8'));
  ...
  it('marks dry_run true in dry-run mode', async () => {
    await withTempImageGenerationDir('test-provenance-dry-run', async outDir => {
      const res = await run({ provider: 'mock', brief: 'hero-key-art-001', output: outDir, dryRun: true });
      const content = JSON.parse(fs.readFileSync(res.provenance, 'utf8'));
